...
var store = function(store){
	store=store||{getItem:function(){},setItem:function(){}};
	function encode(obj){
		if(!obj)return obj;
	...
	var ret={};
	ret.get=function(options){
		options=wrapper(options||{});
		if(!options.key){
			return _apply(options.error,options,["need key!"]);			
		}
		var v = decode(store.getItem(options.key));
		return _apply(options.success,options,[v]);
	}
	ret.set=function(options){
		options=wrapper(options||{});
		if(!options.key){
			return _apply(options.error,options,["need key!"]);			
		}
		var v = encode(options.value);
		store.setItem(options.key,v);
		return _apply(options.success,options,[]);
	}
	ret.remove=function(options){
		options=wrapper(options||{});
		if(!options.key){
			return _apply(options.error,options,["need key!"]);			
		}		
...
define("authority",["jquery","underscore","backbone","m5"],function(require,exports,module){
	...
	_.extend(au.fn,{
		...
		,_login_help:function(options){
			...
			function reloadVC(){
				vcm.attr("src",_op_.vcode+"?t="+Math.random());
			}
		...
		,showform:function(options){
			...
			var _op_={"id":id,"vcode":vcsrc,"ctx":ctx,"storage":!!window.localStorage,name:"",checked:false};
			var that = this;
			store.get({"key":"ulocal",success:function(loc){
				if(loc){
					if(loc.remenberd){
						_op_['checked']=true;
						_op_['name']=loc.name||"";
				...
				var param={"id":id,"model":_op_};
				param = _.extend(param,options);
				that._login_help(param);
			}});
