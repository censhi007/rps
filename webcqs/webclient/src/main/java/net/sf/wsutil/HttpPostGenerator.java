 ...
 *	Generated code uses http POST protocol to connect web service.
 *
 *	@version	0.1
 *
 */
class HttpPostGenerator implements ICodeGenerator {
	 ...
	 */
	public HttpPostGenerator(WSDL wsdl,boolean asynch) {
		this.wsdl = wsdl;
		this.asynch = asynch;
	 ...
	 */
	public HttpPostGenerator(WSDL wsdl) {
		this.wsdl = wsdl;
		try {
			this.ops = wsdl.getOperations();
		} catch (XPathExpressionException e) {
	...
	/**
	 * generate javascript code of sending post message to web service.
	 * 
	 * @param op
	 * @return
	 */
	private String writeFunction(Operation op){
		...
		codes.append(newLine+"\tfunction " + funName
				...
				// function body
				+ "\t\tvar method_url = this.url" + " + \"" + op.httpPostInfo.location + "\"" + newLine
				...
				+ "\t\txmlHttpRequest.open(\"" + op.httpPostInfo.method + "\",method_url," + asynch + ");" + newLine
				+ "\t\txmlHttpRequest.setRequestHeader(\"Content-Type\",\"" + op.httpPostInfo.contentType + "\");" + newLine
				+ "\t\txmlHttpRequest.send(message);" + newLine
