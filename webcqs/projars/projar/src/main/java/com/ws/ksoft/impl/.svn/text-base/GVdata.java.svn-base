...
public class GVdata implements Ti{
		...
		try{
			...
			if(dw!=null){
				if(dw instanceof String){
					...
					dw=db.buildDwdmLike(" v.dwdm",(Collection<Object>)dw);
				}
			}
			...
			for(String s:ndbs){
				...
				qo.clear();
				qo.append(" select v.dwdm,v.reportcode,v.targetcode,v.qskjqj,v.zjkjqj,v.thedata ")
				  .append(" from [").append(s).append("]..gk_v_data v where exists(select 1 from [")
				  .append(s).append("]..gk_v_data_main m where m.dataid=v.expand ");
				if(1 == ty){
				...
				if(0==ty){				
					if(sty!=edy){//开始时间与结束时间不在同一年
						if(edy==-1){//查询大于起始时间的数据
							qo.append(" and v.qskjqj>=:stm").addParam("stm", stm);							
						}else if(sty==-1){//查询小于终止时间的数据
							qo.append(" and v.zjkjqj<=:edm").setParameter("edm", edm);
						}else{
						...
						if(sty==-1){//styy==edy==-1
							...
							//取但年度某期间数据
							qo.append(" and v.zjkjqj<=:edm").setParameter("edm", edm).append(" and v.qskjqj>=:stm").addParam("stm", stm);
						}
					}
				}
				...
				if(rcode!=null){
					qo.append(" and v.reportcode in (:reportcode)").addParam("reportcode", rcode);
				}
			...
			mpo.setState(1);
			mpo.setDescription("@ksoft_vdata_suc");
			return mpo;
		}catch(Throwable e){
			e.printStackTrace();
			mpo.setDescription("@ksoft_vdata_failed", e);
			mpo.setState(0);
			return mpo;
		}
	}
